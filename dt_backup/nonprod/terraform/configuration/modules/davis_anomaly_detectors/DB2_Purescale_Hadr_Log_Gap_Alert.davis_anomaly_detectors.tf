resource "dynatrace_davis_anomaly_detectors" "DB2_Purescale_Hadr_Log_Gap_Alert" {
  description = ""
  enabled     = true
  source      = "Notebooks"
  title       = "DB2 Purescale Hadr Log Gap Alert"
  analyzer {
    ...
    input {
      ...
      analyzer_input_field {
        ...
        value = "timeseries { avg(db2.hadr_log_gap), value.A = avg(db2.hadr_log_gap, scalar: true) }"
      }
      analyzer_input_field {
  ...
  event_template {
    properties {
      ...
      property {
        key   = "event.name"
        value = "DB2 Purescale - Hadr Log Gap Alert"
      }
    }
  }
  execution_settings {

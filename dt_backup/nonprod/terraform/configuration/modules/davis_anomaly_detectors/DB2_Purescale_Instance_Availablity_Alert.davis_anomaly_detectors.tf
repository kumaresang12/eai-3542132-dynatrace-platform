resource "dynatrace_davis_anomaly_detectors" "DB2_Purescale_Instance_Availablity_Alert" {
  description = ""
  enabled     = true
  source      = "Notebooks"
  title       = "DB2 Purescale Instance Availablity Alert"
  analyzer {
    ...
    input {
      ...
      analyzer_input_field {
        ...
        value = "timeseries avg(db2.availability), by: { instance_name }"
      }
      analyzer_input_field {
  ...
  event_template {
    properties {
      ...
      property {
        key   = "event.name"
        value = "DB2 Purescale - Instance Availability Alert"
      }
      property {
        key   = "instance_name"
        value = "{instance_name}"

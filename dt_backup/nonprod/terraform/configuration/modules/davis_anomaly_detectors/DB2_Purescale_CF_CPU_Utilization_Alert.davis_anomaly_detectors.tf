resource "dynatrace_davis_anomaly_detectors" "DB2_Purescale_CF_CPU_Utilization_Alert" {
  description = ""
  enabled     = true
  source      = "Custom Alerts"
  title       = "DB2 Purescale CF CPU Utilization Alert"
  analyzer {
    ...
    input {
      ...
      analyzer_input_field {
        ...
        value =<<-EOT
          timeseries { avg(dt.host.cpu.usage), value.A = avg(dt.host.cpu.usage, scalar: true) }, by: { host.name }, filter: { matchesValue(host.name, { "vrh11057.ute.fedex.com", "vrh11058.ute.fedex.com", "irh11033.ute.fedex.com", "irh11034.ute.fedex.com" }) }
        EOT
      }
      analyzer_input_field {
  ...
  event_template {
    properties {
      ...
      property {
        key   = "event.name"
        value = "DB2 Purescale - CF CPU Utilization Alert"
      }
      property {
        key   = "host.name"
        value = "{host.name}"

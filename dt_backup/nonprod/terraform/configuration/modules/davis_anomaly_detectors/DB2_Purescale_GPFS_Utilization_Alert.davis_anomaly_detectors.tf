resource "dynatrace_davis_anomaly_detectors" "DB2_Purescale_GPFS_Utilization_Alert" {
  description = ""
  enabled     = true
  source      = "Notebooks"
  title       = "DB2 Purescale GPFS Utilization Alert"
  analyzer {
    ...
    input {
      ...
      analyzer_input_field {
        ...
        value =<<-EOT
          timeseries { avg(dt.host.disk.used.percent), value.A = avg(dt.host.disk.used.percent, scalar: true) }, by: { dt.entity.host, dt.entity.disk }, filter: { ( matchesValue(host.name, "vrh11050.ute.fedex.com") AND matchesValue(entityAttr(dt.entity.disk, "entity.name"), "/db2/dbdsnx*") ) OR ( matchesValue(host.name, "vrh11050.ute.fedex.com") AND matchesValue(entityAttr(dt.entity.disk, "entity.name"), "/db2sd") ) OR ( matchesValue(host.name, "vrh11050.ute.fedex.com") AND matchesValue(entityAttr(dt.entity.disk, "entity.name"), "/db2/dba/") ) }
  ...
  event_template {
    properties {
      ...
      property {
        key   = "event.name"
        value = "DB2 Purescale - GPFS Utilization Alert"
      }
      property {
        key   = "host.name"
        value = "{dt.entity.host.name}"
      }
    }
  }
  execution_settings {

resource "dynatrace_davis_anomaly_detectors" "DB2_Purescale_Member_State_Alert" {
  description = ""
  enabled     = true
  source      = "Notebooks"
  title       = "DB2 Purescale Member State Alert"
  analyzer {
    ...
    input {
      ...
      analyzer_input_field {
        ...
        value = "timeseries avg(db2.db_status), by: { member }"
      }
      analyzer_input_field {
  ...
  event_template {
    properties {
      ...
      property {
        key   = "event.name"
        value = "DB2 Purescale - Member State Alert"
      }
      property {
        key   = "member"
        value = "{member}"

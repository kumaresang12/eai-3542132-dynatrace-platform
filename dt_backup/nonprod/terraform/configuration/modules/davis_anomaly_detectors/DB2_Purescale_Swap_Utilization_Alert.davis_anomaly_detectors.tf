resource "dynatrace_davis_anomaly_detectors" "DB2_Purescale_Swap_Utilization_Alert" {
  description = ""
  enabled     = true
  source      = "Notebooks"
  title       = "DB2 Purescale Swap Utilization Alert"
  analyzer {
    ...
    input {
      ...
      analyzer_input_field {
        ...
        value =<<-EOT
          timeseries { avg(dt.host.memory.swap.used), value.A = avg(dt.host.memory.swap.used, scalar: true) }, by: { dt.entity.host }, filter: { matchesValue(host.name, "vrh1105*") OR matchesValue(host.name, "irh1103*") }
  ...
  event_template {
    properties {
      ...
      property {
        key   = "event.name"
        value = "DB2 Purescale - Swap Utilization Alert"
      }
      property {
        key   = "host.name"

resource "dynatrace_davis_anomaly_detectors" "Active_Gate_High_Cpu_Usage" {
  description = ""
  enabled     = true
  source      = "Custom Alerts"
  title       = "Active Gate High Cpu Usage"
  analyzer {
    ...
    input {
      analyzer_input_field {
        ...
        value =<<-EOT
          timeseries {
            cpu_usage = avg(dt.sfm.active_gate.system.cpu_usage)
          }, by: {
            dt.network_zone.id,
  ...
  event_template {
    properties {
      ...
      property {
        key   = "event.name"
        value = "Active Gate High Cpu Usage"
      }
      property {
        key   = "host.name"

resource "dynatrace_davis_anomaly_detectors" "DB2_Purescale_NIC_Received_Packet_Errors_Alert" {
  description = ""
  enabled     = true
  source      = "Custom Alerts"
  title       = "DB2 Purescale NIC Received Packet Errors Alert"
  analyzer {
    ...
    input {
      ...
      analyzer_input_field {
        ...
        value = "timeseries { avg(dt.host.net.nic.packets.errors_rx), value.A = avg(dt.host.net.nic.packets.errors_rx, scalar: true) }, filter: { matchesValue(host.name, \"vrh1105*\") }"
      }
      analyzer_input_field {
  ...
  event_template {
    properties {
      ...
      property {
        key   = "event.name"
        value = "DB2 Purescale - NIC Received Packet Errors Alert"
      }
    }
  }
  execution_settings {

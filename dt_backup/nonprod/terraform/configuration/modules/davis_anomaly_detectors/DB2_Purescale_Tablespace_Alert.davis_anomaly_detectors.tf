resource "dynatrace_davis_anomaly_detectors" "DB2_Purescale_Tablespace_Alert" {
  description = "Non-Prod DB2 Purescale Tablespace State Alert"
  enabled     = true
  source      = "Notebooks"
  title       = "DB2 Purescale Tablespace Alert"
  analyzer {
    ...
    input {
      ...
      analyzer_input_field {
        ...
        value = "timeseries avg(db2.tbsp_state), by: { member }"
      }
      analyzer_input_field {
  ...
  event_template {
    properties {
      ...
      property {
        key   = "event.name"
        value = "DB2 Purescale - Tablespace State Alert"
      }
      property {
        key   = "member"
        value = "{member}"

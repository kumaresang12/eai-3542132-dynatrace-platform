resource "dynatrace_davis_anomaly_detectors" "DB2_Purescale_Members_CPU_Utilization_Alert" {
  description = ""
  enabled     = true
  source      = "Custom Alerts"
  title       = "DB2 Purescale Members CPU Utilization Alert"
  analyzer {
    ...
    input {
      ...
      analyzer_input_field {
        ...
        value =<<-EOT
          timeseries { avg(dt.host.cpu.usage), value.A = avg(dt.host.cpu.usage, scalar: true) }, by: { host.name }, filter: { matchesValue(host.name, { "vrh11050.ute.fedex.com", "vrh11051.ute.fedex.com", "vrh11052.ute.fedex.com", "vrh11053.ute.fedex.com", "vrh11054.ute.fedex.com", "irh11030.ute.fedex.com", "irh11031.ute.fedex.com", "irh11032.ute.fedex.com" }) }
        EOT
      }
      analyzer_input_field {
  ...
  event_template {
    properties {
      ...
      property {
        key   = "event.name"
        value = "DB2 Purescale - Members CPU Utilization Alert"
      }
      property {
        key   = "host.name"
        value = "{host.name}"

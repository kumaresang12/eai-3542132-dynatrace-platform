resource "dynatrace_davis_anomaly_detectors" "DB2_Purescale_Hadr_Standby_Tablespace_Alert" {
  description = ""
  enabled     = true
  source      = "Notebooks"
  title       = "DB2 Purescale Hadr Standby Tablespace Alert"
  analyzer {
    ...
    input {
      ...
      analyzer_input_field {
        ...
        value = "timeseries { avg(db2.hadr_standby_tablespace_error), value.A = avg(db2.hadr_standby_tablespace_error, scalar: true) }, by: { primary_member_host, standby_member_host }"
      }
      analyzer_input_field {
  ...
  event_template {
    properties {
      ...
      property {
        key   = "event.name"
        value = "DB2 Purescale - Hadr Standby Tablespace Alert"
      }
      property {
